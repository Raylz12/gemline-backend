...
`gemline.html` is now a live view over the engine. On load it calls
`POST /api/session` (which seeds a consistent demo world — your listings, a
marketplace, a vault, two incoming trade offers, and a starting credit balance)
and then renders everything from `GET /api/state`. The header pill flips to
**LIVE DATA**. Every action goes to the backend and re-fetches state, so it all
survives a refresh:
- Buy → `POST /api/buy` (vaulted listings settle instantly into your collection; raw ones open an escrowed order)
- Boost → `POST /api/boosts` (debits the ledger, bumps the listing rank)
- Buy credits → `POST /api/credits/purchase`
- Propose / accept / decline / cancel trade → `POST /api/trades…`

Point it at a deployed backend with `gemline.html?api=https://your-host`. When
the backend is unreachable it silently falls back to the built-in demo data,
then to the `/feed` arbitrage data if that's up.

**One deployment note:** browsers block an `https://` page from calling
`http://localhost`, so the live wiring works when you open `gemline.html` from
disk (`file://`) against a local backend, or when both the page and the API are
served over `https`. The standalone demo always works regardless.
